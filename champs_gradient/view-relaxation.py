...
if len(sys.argv) <4 :
    print()
    print('Usage:')
    print('  {} <root_dir> <timestep> <time_inp> <i>'.format(sys.argv[0]))
    print()
    sys.exit(0)

root_dir = sys.argv[1]
timestep = int(sys.argv[2])
time_inp = int(sys.argv[3])
i = int(sys.argv[4])
timeline_prefix = 'zrlx-{:04d}-{:04d}-{:03d}-'.format(timestep,time_inp,i)
...
slider = cx.tkviewer.HistoryFromVariableSlider(root,
                                               'Relaxation step',
                                               cx.variable.path_from(root_dir, rlx_timeline, 'M1/BMU'))

hbox = tk.Frame(root)
v1   = MapView(hbox, 'M1', 'M2', timestep)
v2   = MapView(hbox, 'M2', 'M1', timestep)
