...
auto make_architecture(int nmaps, int walltime){
...
auto make_closed_architecture(int nmaps,int map_closed){
  ...
  p_external   | p_main;
  p_contextual | p_main;
  p_global     | p_main,  kwd::use("random-bmu", 1), kwd::use("beta", BETA), kwd::use("delta", .2), kwd::use("deadline", DEADLINE);
...
if(c.user_argv[0]== "main"){
  ...
  auto archi = make_architecture(nmaps, WALLTIME);
  archi->realize();
  ...
  auto archi = make_architecture(nmaps, -1);
  ...
  unsigned int timestep = stoi(c.user_argv[2]);
  int nmaps = stoi(c.user_argv[3]);
  auto archi = make_architecture(nmaps, 10);
