...
def count_relax(conv):
    return len(list(filter(lambda x:x<200,conv)))/len(conv) * 100
...
if __name__ == "__main__":
    if len(sys.argv)<2:
        sys.exit("usage: python3 plot_weights.py <data_dir>")
    dirs= sys.argv[1:]
    print(dirs)
    #timestep = int(sys.argv[2])
    """
    with cx.variable.Realize(os.path.join(dir,'zfrz-%08d-rlx'%timestep,'Cvg.var')) as cvg:
