...
if __name__ == "__main__":
    ...
    for i,m in enumerate(maps):
        ...
        wfc.close()
        test_name = "zfrz-%08d-out" %timestep
        with(cx.variable.Realize(os.path.join(dir,test_name,m,'BMU.var'))) as bmu:
            bmus = [bmu[i] for i in range(bmu.time_range()[1])]
            print(bmus)
            ax[row,col].scatter([b[0] for b in bmus],[b[1] for b in bmus])
            ax2[i,0].scatter([b[0] for b in bmus],[b[1] for b in bmus])
